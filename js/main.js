...
const sunTransformText = $('.welcome-sun__transform')
  .find('.chat-cloud__p')
  .text();
...
function initLoader() {
  ...
  tl.set($('.welcome-sun__transform'), {
    y: () => {
      ...
      const top =
        $('.welcome-sun__transform').offset().top - $(window).scrollTop();
      return (h - elH) / 2 - top;
    },
  });
  ...
  tl.from(
    $('.welcome__h1 > span, .welcome__h2-box-wrap'),
    {
      duration: 1,
      yPercent: 100,
      autoAlpha: 0,
...
function initStackedCardsDrag() {
  $('[data-stacked-cards]').each(function () {
    ...
    function restack() {
      ...
      $items
        .slice(3)
        .css({ 'z-index': 0, transform: `rotate(${inactiveDeg})` });
    }
    ...
    function resetCycle() {
      $list
        .find('[data-stacked-cards-card].is--dragging')
        .removeClass('is--dragging');
      setupDraggables();
    }
...
function initStickyCursor() {
  ...
  document
    .querySelectorAll('[data-sticky-cursor]')
